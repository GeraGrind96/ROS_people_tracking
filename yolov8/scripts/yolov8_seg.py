...
class yolov8():
    ...
    def get_yolo_objects(self, event):
        if self.new_data:
            ...
            init = time.time()
            results = self.model.predict(self.color_image, classes=0)
            res_plotted = results[0].plot()
            for result in results:
                masks = result.masks
                for mask in masks:
                    self.get_mask_distance(mask, self.depth_image)
    ...
    def get_mask_distance(self, mask, depth_image):
        mask_depths = []
        for point in mask:
            act_depth = depth_image[int(point[1])][int(point[0])]
            if not math.isinf(act_depth):
                mask_depths.append(act_depth)
        if len(mask_depths) > 0:
            mask_depth_mean = sum(mask_depths) / len(mask_depths)
            print("DEPTH:", mask_depth_mean)
